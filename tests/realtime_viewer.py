...
def multi_slice_viewer_debug(
    ...
    *gt_volumes: Tensor,
    ...
    # assertion part:
    assert _is_tensor(img_volume) or _is_iterable_tensor(img_volume), f"input wrong for img_volume, given {img_volume}."
    assert _is_iterable_tensor(gt_volumes) or gt_volumes == (), f"input wrong for gt_volumes, given {gt_volumes}."
    ...
    for _row_num, row_axs in enumerate(axs):
        ...
        for _col_num, ax in enumerate(row_axs):
            ...
            ax.gt_volume = None if _empty_iterator(gt_volumes) else tensor2plotable(gt_volumes[_col_num])
            try:
                if not no_contour:
                    ax.con = ax.contour(ax.gt_volume[ax.index])
                else:
